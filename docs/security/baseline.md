...
| **C5** | *"A Vercel firewall rule rate-limits the AI routes, and the AI provider has a monthly spend cap. Without sign-in, anyone with the link could otherwise run up AI costs."* | [#93](https://github.com/dczii/URecruitment/issues/93) E01-S04-T02 (one `/api/ai/*` prefix; the firewall rule, which Hobby offers, committed as `infra/vercel/ai-rate-limit.rule.json`; the provider-side cap procedure with an owner) · [#175](https://github.com/dczii/URecruitment/issues/175) E11-S04-T01 (app-side cap in `runAi()`; the app-level per-route limiter; records which rate-limit control is in force) | `src/server/ai/spend-cap.test.ts` (under cap proceeds; at cap refused and recorded; Singapore month boundary; non-AI pages still work) · **Gap, assigned to #175:** a test that a request over the limit on an `/api/ai/*` route is refused with a clear message · `test/infra/ai-route-prefix.test.ts` (an AI-importing route handler outside `/api/ai/` fails) · `test/infra/ai-rate-limit-rule.test.ts` (the committed rule matches the prefix and Hobby's limits) · `src/lib/ai-routes.test.ts` (a 429 maps to the recruiter message) · manual: the burst probe in the [infrastructure plan](../plans/infrastructure.md#rate-limit-and-spend-cap-93) once the rule is published |
...
**About C5's firewall rule.** The PRD names a Vercel firewall rule.
[#93](https://github.com/dczii/URecruitment/issues/93) checked on 2026-09-18 that Hobby offers **one**
rate-limit rule per project. It committed that rule as `infra/vercel/ai-rate-limit.rule.json`, so the
firewall rule is the control in force. A person publishes it before the first AI route reaches
production ([infrastructure plan](../plans/infrastructure.md#rate-limit-and-spend-cap-93)).
...
| AI routes live under **one** prefix, `/api/ai/*`, and are rate-limited (C5) | [#93](https://github.com/dczii/URecruitment/issues/93) | `test/infra/ai-route-prefix.test.ts`, `test/infra/ai-rate-limit-rule.test.ts` |
