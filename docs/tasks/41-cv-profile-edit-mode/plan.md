...
- [ ] **AC1** — A recruiter can edit any parsed field and the change persists through a re-parse. _Proved by:_ `candidate.spec.ts › AC1: edits a field and it persists`, reusing #132's `setOverride` (already proven to survive re-parse at the unit level in `overrides.test.ts`)
- [ ] **AC2** — The name prompt appears before the first change on a device and not again afterward. _Proved by:_ `candidate.spec.ts › AC2: name prompt appears once per device`
- [ ] **AC3** — An edited field is visually distinguishable and the original parsed value is still viewable. _Proved by:_ manual inspection — `FieldCard`'s existing edited-diff rendering (#134) already satisfies this; this task adds the name to the "Edited by" tag
- [ ] **AC4** — `candidate-profile.test.ts` covers the new `overridden_by`/`full_name` fields. _Proved by:_ `candidate-profile.test.ts › exposes overridden_by and full_name`
...
- [ ] **T1a** `grok` — Failing tests first in `candidate-profile.test.ts`: the query now returns `full_name` and, for an overridden field, `overridden_by`/`overridden_at`.
  - Verify: `npm test -- candidate-profile` → fails (new assertions on missing fields)
- [ ] **T1b** `grok` — Extend `candidate-profile.ts` until T1a passes.
  - Verify: `npm test -- candidate-profile` → pass; `npm run typecheck`
- [ ] **T2** `grok` — Add the edit affordance + `saveFieldOverride` Server Action + typed-name gate to `CandidateProfile.tsx`/`actions.ts`; update `e2e/candidate.spec.ts` with AC1/AC2 coverage (desktop project only).
