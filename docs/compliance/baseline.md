...
| Real-data requirement | Access logging tied to a real identity (which depends on OQ-1); a procedure that decides within 3 calendar days whether a breach is notifiable. The PRD notes that *"At up to 1,000 CVs a month, the database passes 500 people quickly"* |
...
| Missing reason | A nationality or language requirement without a written reason is refused at save, so it never reaches the gap check as a silent preference (`compliance-review` §B) | **Decided** (the reason rule) | [#135](https://github.com/dczii/URecruitment/issues/135) |
