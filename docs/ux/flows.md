...
| 1.2 | S4 Job form | R | *(Optional)* uploads the client's JD (PDF or Word) | S: the file is validated (type by content, size ≤ 50 MB) and stored in the **private** bucket → `cv_files` row (kind `jd`). A scanned or image-only file is **rejected with a clear message**. S: one AI call extracts the fields → `ai_runs` row |
...
| 1.5 | S4 Job form | R | **Save** → S11 on first change | S: `jobs` row (new job) and a new immutable **`job_versions`** row; the `jobs` current-version pointer → it. S: **gap check**: code rules for missing fields plus one AI call for uncertain, conflicting and fair-employment flags → `gap_flags` rows, `ai_runs` row |
| 1.6 | — | S | *After the response is sent* (`after()`): re-score candidates for the new version | S: embeddings for the version; retrieval (filters, then top 50 by similarity); one scoring call per candidate → **`match_scores`** keyed to *candidate × job version × model version*, `ai_runs` rows; progress in the re-score runs table ([#150](https://github.com/dczii/URecruitment/issues/150); not one of ADR-0002's seventeen) so a failed run can be retried |
...
| 1.13 | S3 Job detail | R | **Add to pipeline** for the candidates they choose. **Nothing is pre-selected** → S11 | `pipeline_entries` row in **Sourced** with `entered_at` = now ([#156](https://github.com/dczii/URecruitment/issues/156)). *Proposed here, for [#151](https://github.com/dczii/URecruitment/issues/151) and #156 to confirm:* a `stage_events` row with the typed name, since entering Sourced starts the stage clock |
...
`embeddings` · `match_scores` · the re-score runs table · `pipeline_entries` · `stage_events`.
...
| 2.9 | S7 → S8 Placements | R | *(If the move was to **Placed**)* confirms the candidate's **start date**, on Placements ([#164](https://github.com/dczii/URecruitment/issues/164); action [#162](https://github.com/dczii/URecruitment/issues/162)) → S11 | `placements` row with the start date and the **guarantee end date** (the client's period, default 30 days); Placed shows no delay status |
...
| 3.5 | S1 → S3 Job detail | R | For a candidate **waiting on the client** (Submitted to client; Client interview, which may also wait on the candidate): opens the job and chases the client **outside the portal**. The row stays overdue until the client acts and the recruiter records the move | none. The "waiting on" column keeps client-side delays visibly apart |
