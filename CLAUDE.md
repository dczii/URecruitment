...
- **`/task <description | #issue>`** runs the `urec-orchestrator` skill. Use it instead of the global `orchestrator` skill in this repo. It writes `docs/tasks/<issue>-<slug>/spec.md` and `plan.md`, has Cursor Grok 4.6 or GPT-5.6 (`cursor-agent`) implement, has Claude verify and review, then opens a PR and moves the card to In Review. There is no approval gate. **Claude never merges.**
