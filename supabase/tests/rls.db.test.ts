...
const LOCAL_STACK_REQUIRED =
  "Start the local stack with `supabase start`, then export SUPABASE_URL (API_URL) and SUPABASE_PUBLISHABLE_KEY (PUBLISHABLE_KEY) from `supabase status -o env`. Auth is disabled in config.toml and status prints the keys only while auth runs, so start the stack with `SUPABASE_AUTH_ENABLED=true supabase start` (as CI does). DB tests never run against a remote project.";
