---
description: Plan and build one task end to end (issue → plan → Cursor executor implements → verify → PR)
...
- Write `docs/tasks/<issue>-<slug>/plan.md` before any code changes. Do not create `spec.md`.
- Code is written by `cursor-agent` (Cursor Grok 4.6 or GPT-5.6) with the binding skill rules inlined. `.pen` design steps are done by Claude.
